  ...
  xhr.onreadystatechange = function() {
    ...
    if (xhr.status != 200) {
      ...
      try {
        ...
        alert("Что-то пошло не так. Вероятно вы не авторизированы");
      }
...
chrome.runtime.onMessage.addListener(function (request) {
  if (request.cmd === "getCode") {
    ...
    var filter = `{"Ваш код Uber" "Your Uber code"} is:unread after:${date} from:noreply@zadarma.com`
                        ...
                        if (obj.body.resultSizeEstimate) {
                          ...
                          setTimeout(() => {
                            getGmail();
                                ...
                                console.log(code[0]);
                                chrome.storage.sync.set({code: code[0]}, function() {
                                })
                                ...
                                body: JSON.stringify({'removeLabelIds': ['UNREAD']})
                                      ...
                                      .then(data=> data)
                                          .then(obj=>{console.log(obj)
                                            chrome.storage.sync.set({getMessageFlag: 1}, function() {
                                            })
                                            console.log('getMessageFlag in the end from background.js');
